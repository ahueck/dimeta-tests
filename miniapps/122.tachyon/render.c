 ...
 *
 *  $Id: render.c 1048 2008-06-11 17:47:00Z keeper $
 */
...
static void renderio(scenedef * scene) {
  ...
  rt_timer_start(ioth);
...
#ifdef SPEC_MPI
   {
      ...
      int cxres = 100; /* crop the image to a 100x100 area */
      int cyres = 100;
      imgcrop = image_crop_rgb24(scene->hres, scene->vres, scene->img,
                                 cxres, cyres,
                                 scene->hres/2-50, scene->vres/2-50);
      writeimage(scene->outfilename, cxres, cyres,
                 imgcrop, scene->imgformat);
      free(imgcrop);
    }
#else 
