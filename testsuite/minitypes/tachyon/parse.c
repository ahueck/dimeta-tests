// RUN: %apply-dimeta-mpicc -Iinclude/ %s |& %filecheck %s

// Check all malloc calls in this TU:

// CHECK-LABEL: reset_tex_table:
// CHECK-NEXT:  0 | texentry
// CHECK-NEXT:  0 |   char * name
// CHECK-NEXT:  8 |   void * tex
// CHECK-NEXT:    | [sizeof=16]

// CHECK-LABEL: GetPolyCylinder:
// CHECK-NEXT:  0 | apivector
// CHECK-NEXT:  0 |   apiflt x
// CHECK-NEXT:  8 |   apiflt y
// CHECK-NEXT: 16 |   apiflt z
// CHECK-NEXT:    | [sizeof=24]

// CHECK-LABEL: GetClipGroup:
// CHECK-NEXT:  | float *
// CHECK-NEXT:  | [sizeof=4]

// CHECK-LABEL: add_texture:
// CHECK-NEXT:  | char *
// CHECK-NEXT:  | [sizeof=1]
 ...
 *
 *  $Id: parse.c 499 2006-10-06 19:58:28Z keeper $
 */
...
static errcode add_texture(parsehandle *ph, void *tex, const char *name) {
  ...
  rt_hash_insert(&ph->texhash, ph->textable[ph->numtextures].name,
                 ph->numtextures);
...
static void PrintSyntaxError(parsehandle *ph, const char *string,
                             const char *found) {
...
static errcode ReadIncludeFile(parsehandle *ph, const char *includefile,
                               SceneHandle scene) {
  errcode rc;
...
static errcode GetCamera(parsehandle *ph, SceneHandle scene) {
  ...
  rt_camera_setup(scene, zoom, aspectratio, antialiasing, raydepth, Ccenter,
                  Cview, Cup);
...
static errcode GetSpotLight(parsehandle *ph, SceneHandle scene) {
  ...
  if (!stringcmp(tmp, "COLOR")) {
    ...
    li = rt_spotlight(scene, rt_texture(scene, &tex), ctr, rad, direction,
                      start, end);
  } else {
    if (stringcmp(tmp, "ATTENUATION"))
      return -1;
