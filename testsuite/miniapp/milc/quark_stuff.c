// RUN: %apply-dimeta-mpicc -Iinclude/ -std=c90 %s |& %filecheck %s

// CHECK: path_transport:
// CHECK-NEXT:         0 | su3_vector
// CHECK-NEXT:         0 |   complex[3] c
...
// CHECK: path_transport_hwv:
// CHECK-NEXT:         0 | half_wilson_vector
// CHECK-NEXT:         0 |   su3_vector[2] h
...
// CHECK: compute_gen_staple:
// CHECK-NEXT:         0 | su3_matrix
// CHECK-NEXT:         0 |   complex[9] e
// CHECK-NEXT:           | [sizeof=72]

// CHECK: eo_fermion_force:
// CHECK-NEXT:         0 | su3_vector
// CHECK-NEXT:         0 |   complex[3] c
// CHECK-NEXT:           | [sizeof=24]
...
// CHECK: u_shift_fermion:
// CHECK-NEXT:         0 | su3_vector
// CHECK-NEXT:         0 |   complex[3] c
// CHECK-NEXT:           | [sizeof=24]

// CHECK: eo_fermion_force_3f:
// CHECK-NEXT:         0 | half_wilson_vector
// CHECK-NEXT:         0 |   su3_vector[2] h
// CHECK-NEXT:           | [sizeof=48]
...
// CHECK: u_shift_hw_fermion:
// CHECK-NEXT:         0 | half_wilson_vector
// CHECK-NEXT:         0 |   su3_vector[2] h
// CHECK-NEXT:           | [sizeof=48]

#define DSLASH_TMP_LINKS
#define CONGRAD_TMP_VECTORS
#define FAST
#define FN
...
#define NULL_FP                                                                \
  -1 /* NULL field_offset to be used in the optimized version *                \
...
void u_shift_hw_fermion(half_wilson_vector *src, half_wilson_vector *dest,
                        int dir);
void add_3f_force_to_mom(half_wilson_vector *back, half_wilson_vector *forw,
                         int dir, float coeff[2]);
void side_link_3f_force(int mu, int nu, float coeff[2],
                        half_wilson_vector *Path, half_wilson_vector *Path_nu,
      ...
      for (perm[2] = 0; perm[2] < 4; perm[2]++)
        for (perm[3] = 0; perm[3] < 4; perm[3]++) {
          if (perm[0] != perm[1] && perm[0] != perm[2] && perm[0] != perm[3] &&
              perm[1] != perm[2] && perm[1] != perm[3] && perm[2] != perm[3]) {
            /* reflections*/
                ...
                for (ir[2] = 0; ir[2] < 2; ir[2]++)
                  for (ir[3] = 0; ir[3] < 2; ir[3]++) {
                    ...
                    if (flag == 0) {
                      ...
                      /* remember to copy NODIR's, or comparison will get
                       * confused */
                      if (ir[0] == 0) {
                      ...
                      num_q_paths++;
                      path_num++;
                      /**node0_printf("ADD PATH %d:  rx=%d
                         ",num_q_paths-1,ir[0]); printpath( vec, length );**/
                    }
...
void path_transport(field_offset src, field_offset dest, int parity, int *dir,
  ...
  if (length > 0) {
    ...
    for (j = length - 1; j >= 0; j--) {
      ...
      if (j % 2 == 0) {
        ...
        case EVEN:
          ...
          break;
        case ODD:
          tmp_otherparity = EVEN;
          break;
        case EVENANDODD:
          tmp_otherparity = EVENANDODD;
          break;
        }
      } else { /* odd # step */
      ...
      if (GOES_FORWARDS(dir[j])) {
        mtag0 = start_gather_from_temp(tmp_src, sizeof(su3_vector), dir[j],
                                       tmp_parity, gen_pt[0]);
        wait_gather(mtag0);
        ...
        FORSOMEPARITY(i, s, tmp_otherparity) {
          mult_adj_su3_mat_vec(&(s->link[OPP_DIR(dir[j])]), &(tmp_src[i]),
                               &(tmp_work[i]));
        }
        mtag0 = start_gather_from_temp(tmp_work, sizeof(su3_vector), dir[j],
                                       tmp_parity, gen_pt[0]);
        wait_gather(mtag0);
        FORSOMEPARITY(i, s, tmp_parity) {
    ...
    FORSOMEPARITY(i, s, parity) { *(su3_vector *)F_PT(s, dest) = tmp_src[i]; }
    free(tmp_src);
    free(tmp_dest);
    free(tmp_work);
  } /* end if(length>0) */
...
void path_transport_hwv(field_offset src, field_offset dest, int parity,
  ...
  msg_tag *mtag0;
  int j;
  half_wilson_vector *tmp_src, *tmp_dest,
      *tmp_work;                   /*source, dest and workspace*/
  half_wilson_vector *tmp_pt;      /* scratch */
  int tmp_parity, tmp_otherparity; /* parity for this step */

  if (length > 0) {
    tmp_src = (half_wilson_vector *)malloc(sites_on_node *
                                           sizeof(half_wilson_vector));
    tmp_dest = (half_wilson_vector *)malloc(sites_on_node *
                                            sizeof(half_wilson_vector));
    tmp_work = (half_wilson_vector *)malloc(sites_on_node *
                                            sizeof(half_wilson_vector));
    ...
    for (j = length - 1; j >= 0; j--) {
      ...
      if (GOES_FORWARDS(dir[j])) {
        mtag0 = start_gather_from_temp(tmp_src, sizeof(half_wilson_vector),
        ...
        FORSOMEPARITY(i, s, tmp_otherparity) {
          mult_adj_su3_mat_hwvec(&(s->link[OPP_DIR(dir[j])]), &(tmp_src[i]),
                                 &(tmp_work[i]));
        }
        mtag0 = start_gather_from_temp(tmp_work, sizeof(half_wilson_vector),
                                       dir[j], tmp_parity, gen_pt[0]);
        wait_gather(mtag0);
        FORSOMEPARITY(i, s, tmp_parity) {
...
void dslash_eo(field_offset src, field_offset dest, int parity) {
  ...
  */
  ...
  for (ipath = 0; ipath < num_q_paths; ipath++) { /* loop over paths */
    path_transport(src, F_OFFSET(tempvec[0]), parity, q_paths[ipath].dir,
                   q_paths[ipath].length);
    x = q_paths[ipath].coeff;
    FORSOMEPARITY(i, s, parity) {
      scalar_mult_add_su3_vector((su3_vector *)F_PT(s, dest), &(s->tempvec[0]),
...
void load_longlinks() {
  ...
  for (dir = XUP; dir <= TUP; dir++) { /* loop over longlink directions */
    ...
    for (ipath = 0; ipath < num_q_paths; ipath++) { /* loop over paths */
      ...
      for (disp[dir] += 3, i = XUP; i <= TUP; i++)
        if (disp[i] != 0)
          break;
      if (i <= TUP)
        continue; /* skip if path doesn't go to right place */
      ...
      FORALLSITES(i, s) {
        ...
        long1 = &(s->longlink[dir]);
#endif
        scalar_mult_add_su3_matrix(long1, &(s->staple), -q_paths[ipath].coeff,
                                   long1);
...
void load_fatlinks() {
...
#ifdef ASQ_OPTIMIZED_FATTENING
  int nu, rho, sig;
  float one_link; /*needed to fix the problem with the Lepage term */
#else
  int ipath;
  int disp[4];
#endif
  ...
  for (dir = XUP; dir <= TUP; dir++) { /* loop over fatlink directions */
    ...
    for (ipath = 0; ipath < num_q_paths; ipath++) { /* loop over paths */
      ...
      for (disp[dir] += 1, i = XUP; i <= TUP; i++)
        if (disp[i] != 0)
          break;
      if (i <= TUP)
        continue; /* skip if path doesn't go to right place */
                  /**printf("dir = %d, found a path:  ",dir);
                  for(j=0;j<q_paths.[ipath].length;j++)printf("\t%d",
      ...
      FORALLSITES(i, s) {
        ...
        fat1 = &(s->fatlink[dir]);
#endif
        scalar_mult_add_su3_matrix(fat1, &(s->staple), -q_paths[ipath].coeff,
                                   fat1);
        /* minus sign in coeff. because we used backward path*/
      }
    } /* ipath */
          ...
          if ((rho != dir) && (rho != nu)) {
            ...
            for (sig = XUP; sig <= TUP; sig++)
              if ((sig != dir) && (sig != nu) && (sig != rho)) {
                compute_gen_staple(NULL_FP, dir, sig, F_OFFSET(tempmat1),
                                   act_path_coeff[4]);
              } /* sig */
          } /* rho */
      } /* nu */
...
void eo_fermion_force(float eps, int nflavors, field_offset x_off) {
  ...
  for (ipath = 0; ipath < num_q_paths; ipath++) {
    ...
    path_transport(x_off, F_OFFSET(tempvec[0]), EVENANDODD, q_paths[ipath].dir,
                   length);
    ...
    for (ilink = 0; ilink <= length; ilink++) {
      ...
      if ((ilink > 0 && ilink < length) ||
        ...
        if (GOES_FORWARDS(lastdir)) {
          FORALLSITES(i, s) {
            mult_adj_su3_mat_hwvec(&(s->link[lastdir]), &(hw_tmp0[i]),
                                   &(hw_tmp1[i]));
          }
          mtag0 =
              start_gather_from_temp(hw_tmp1, 2 * sizeof(su3_vector),
          ...
          mtag0 =
              start_gather_from_temp(hw_tmp0, 2 * sizeof(su3_vector),
                                     OPP_DIR(lastdir), EVENANDODD, gen_pt[0]);
          wait_gather(mtag0);
          FORALLSITES(i, s) {
...
void eo_fermion_force_3f(float eps, int nflav1, field_offset x1_off, int nflav2,
                         field_offset x2_off) {
  /* note CG_solution and Dslash * solution are combined in "x_off" */
  /* New version 1/21/99.  Use forward part of Dslash to get force */
  ...
  w_tmp0 = (wilson_vector *)malloc(sites_on_node * sizeof(wilson_vector));
  w_tmp1 = (wilson_vector *)malloc(sites_on_node * sizeof(wilson_vector));
  ...
  for (ipath = 0; ipath < num_q_paths; ipath++) {
    ...
    **/
    ...
    if (x2_off - x1_off != sizeof(su3_vector)) {
      node0_printf("BOTCH\n");
      exit(0);
    }
    path_transport_hwv(x1_off, F_OFFSET(tempvec[0]), EVENANDODD,
                       q_paths[ipath].dir, length);
    ...
    for (ilink = 0; ilink <= length; ilink++) {
      ...
      if ((ilink > 0 && ilink < length) ||
        ...
        if (GOES_FORWARDS(lastdir)) {
          FORALLSITES(i, s) {
            mult_adj_mat_wilson_vec(&(s->link[lastdir]), &(w_tmp0[i]),
                                    &(w_tmp1[i]));
          }
          mtag0 =
              start_gather_from_temp(w_tmp1, 4 * sizeof(su3_vector),
                                     OPP_DIR(lastdir), EVENANDODD, gen_pt[0]);
          wait_gather(mtag0);
          FORALLSITES(i, s) { w_tmp0[i] = *(wilson_vector *)gen_pt[0][i]; }
          cleanup_gather(mtag0);
        } else { /* GOES_BACKWARDS(lastdir) */
          mtag0 =
              start_gather_from_temp(w_tmp0, 4 * sizeof(su3_vector),
                                     OPP_DIR(lastdir), EVENANDODD, gen_pt[0]);
          wait_gather(mtag0);
          FORALLSITES(i, s) {
            mult_mat_wilson_vec(&(s->link[OPP_DIR(lastdir)]),
                                (wilson_vector *)(gen_pt[0][i]), &(w_tmp1[i]));
          }
          tmp_pt = w_tmp0;
          w_tmp0 = w_tmp1;
          w_tmp1 = tmp_pt;
      ...
      if ((mu != sig) && (mu != OPP_DIR(sig))) {
        ...
        for (nu = 0; nu < 8; nu++)
          if ((nu != mu) && (nu != OPP_DIR(mu)) && (nu != sig) &&
            ...
            for (rho = 0; rho < 8; rho++)
              if ((rho != mu) && (rho != OPP_DIR(mu)) && (rho != nu) &&
                  (rho != OPP_DIR(nu)) && (rho != sig) &&
                  (rho != OPP_DIR(sig))) {
            ...
            side_link_force(nu, sig, -FiveSt, Pmu, P5, Pnumu, P5nu);
...
void eo_fermion_force_3f(float eps, int nflav1, field_offset x1_off, int nflav2,
  ...
  /* Allocate temporary vectors */
  for (mu = 0; mu < 8; mu++)
    hwvec[mu] = (half_wilson_vector *)malloc(sites_on_node *
                                             sizeof(half_wilson_vector));
  ...
  for (sig = 0; sig < 8; sig++) {
    for (mu = 0; mu < 8; mu++)
      if ((mu != sig) && (mu != OPP_DIR(sig))) {
          ...
          if ((nu != mu) && (nu != OPP_DIR(mu)) && (nu != sig) &&
            ...
            for (rho = 0; rho < 8; rho++)
              if ((rho != mu) && (rho != OPP_DIR(mu)) && (rho != nu) &&
                ...
                side_link_3f_force(rho, sig, SevenSt, Pnumu, P7, Prhonumu,
                                   P7rho);
                /* Add the P7rho vector to P5 */
                coeff[0] = SevenSt[0] / FiveSt[0];
                coeff[1] = SevenSt[1] / FiveSt[1];
                FORALLSITES(i, s) {
                  scalar_mult_add_su3_vector(&(P5[i].h[0]), &(P7rho[i].h[0]),
                                             coeff[0], &(P5[i].h[0]));
                  scalar_mult_add_su3_vector(&(P5[i].h[1]), &(P7rho[i].h[1]),
                                             coeff[1], &(P5[i].h[1]));
                }
              } /* rho */
            /* Length 5 paths */
            ...
            side_link_3f_force(nu, sig, mFiveSt, Pmu, P5, Pnumu, P5nu);
            /* Add the P5nu vector to P3 */
            coeff[0] = FiveSt[0] / ThreeSt[0];
            coeff[1] = FiveSt[1] / ThreeSt[1];
            FORALLSITES(i, s) {
              scalar_mult_add_su3_vector(&(P3[i].h[0]), &(P5nu[i].h[0]),
                                         coeff[0], &(P3[i].h[0]));
              scalar_mult_add_su3_vector(&(P3[i].h[1]), &(P5nu[i].h[1]),
                                         coeff[1], &(P3[i].h[1]));
            }
          } /* nu */
        ...
        FORALLSITES(i, s) {
          scalar_mult_add_su3_vector(&(P3[i].h[0]), &(P5nu[i].h[0]), coeff[0],
                                     &(P3[i].h[0]));
          scalar_mult_add_su3_vector(&(P3[i].h[1]), &(P5nu[i].h[1]), coeff[1],
                                     &(P3[i].h[1]));
        }
...
BOMB THE COMPILE
#endif
    void
    compute_gen_staple(field_offset staple, int mu, int nu, field_offset link,
                       float coef) {
  ...
  */
  ...
  mtag1 = start_gather(F_OFFSET(link[nu]), sizeof(su3_matrix), mu, EVENANDODD,
                       gen_pt[1]);
  wait_gather(mtag0);
  wait_gather(mtag1);
  ...
  mtag0 = start_gather(F_OFFSET(link[nu]), sizeof(su3_matrix), mu, EVENANDODD,
                       gen_pt[0]);
  wait_gather(mtag0);
  FORALLSITES(i, s) {
  ...
  cleanup_gather(mtag0);
  mtag0 = start_gather_from_temp(tempmat, sizeof(su3_matrix), OPP_DIR(nu),
                                 EVENANDODD, gen_pt[0]);
  wait_gather(mtag0);
  ...
  if (staple != NULL_FP) { /* Save the staple */
    FORALLSITES(i, s) {
      ...
      fat1 = &(s->fatlink[mu]);
#endif
      scalar_mult_add_su3_matrix(fat1, (su3_matrix *)F_PT(s, staple), coef,
                                 fat1);
    }
  } else { /* No need to save the staple. Add it to the fatlinks */
    FORALLSITES(i, s) {
...
void u_shift_fermion(su3_vector *src, su3_vector *dest, int dir) {
  ...
  {
    mtag = start_gather_from_temp(src, sizeof(su3_vector), dir, EVENANDODD,
                                  gen_pt[0]);
    wait_gather(mtag);
    FORALLSITES(i, s)
  ...
  } else /* backward shift */
  {
    ...
    mtag = start_gather_from_temp(tmpvec, sizeof(su3_vector), dir, EVENANDODD,
                                  gen_pt[0]);
    wait_gather(mtag);
...
void u_shift_hw_fermion(half_wilson_vector *src, half_wilson_vector *dest,
  ...
  if (GOES_FORWARDS(dir)) /* forward shift */
  {
    mtag = start_gather_from_temp(src, sizeof(half_wilson_vector), dir,
                                  EVENANDODD, gen_pt[0]);
    wait_gather(mtag);
    FORALLSITES(i, s)
  ...
  {
    tmpvec = (half_wilson_vector *)malloc(sites_on_node *
                                          sizeof(half_wilson_vector));
    FORALLSITES(i, s)
...
/* Add in contribution to the force */
/* Put antihermitian traceless part into momentum */
void add_force_to_mom(su3_vector *back, su3_vector *forw, int dir,
                      float coeff) {
  register site *s;
  register int i;
...
void side_link_3f_force(int mu, int nu, float coeff[2],
  ...
  if (GOES_FORWARDS(mu)) {
    ...
    if (GOES_FORWARDS(nu))
      ...
      add_3f_force_to_mom(Path, Path_numu, OPP_DIR(mu),
                          m_coeff); /* ? extra - */
  } else                            /*GOES_BACKWARDS(mu)*/
  {
    /* Add the force :  o    o         *
