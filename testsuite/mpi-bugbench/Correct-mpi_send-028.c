...
int main(int argc, char **argv) {
  ...
  double _Complex *buf_mpi_c_double_complex =
      (double _Complex *)calloc(1, sizeof(double _Complex));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_c_double_complex, 1, MPI_C_DOUBLE_COMPLEX, 1, 0,
             MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_c_double_complex, 1, MPI_C_DOUBLE_COMPLEX, 0, 0,
             MPI_COMM_WORLD);
  }
  free(buf);
