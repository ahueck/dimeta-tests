...
int main(int argc, char **argv) {
  ...
  _Bool *buf_mpi_c_bool = (_Bool *)calloc(1, sizeof(_Bool));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_c_bool, 1, MPI_C_BOOL, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_c_bool, 1, MPI_C_BOOL, 0, 0, MPI_COMM_WORLD);
  }
  free(buf);
