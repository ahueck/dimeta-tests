...
/* ///////////////////////// The MPI Bug Bench ////////////////////////
  ...
  Description: datatype missmatch: Buffer: MPI_INT MPI_Call: MPI_UNSIGNED_CHAR
...
int main(int argc, char **argv) {
  ...
  unsigned char *buf_mpi_unsigned_char =
      (unsigned char *)calloc(10, sizeof(unsigned char));

  if (rank == 0) {
    /*MBBERROR_BEGIN*/ MPI_Irecv(buf_mpi_int, 10, MPI_UNSIGNED_CHAR, 1, 0,
                                 MPI_COMM_WORLD,
                                 &mpi_request_0); /*MBBERROR_END*/
    MPI_Wait(&mpi_request_0, MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    /*MBBERROR_BEGIN*/ MPI_Isend(buf_mpi_int, 10, MPI_UNSIGNED_CHAR, 0, 0,
                                 MPI_COMM_WORLD,
                                 &mpi_request_0); /*MBBERROR_END*/
    MPI_Wait(&mpi_request_0, MPI_STATUS_IGNORE);
  }
