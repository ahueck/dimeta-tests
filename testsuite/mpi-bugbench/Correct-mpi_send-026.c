...
int main(int argc, char **argv) {
  ...
  float _Complex *buf_mpi_c_complex =
      (float _Complex *)calloc(1, sizeof(float _Complex));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_c_complex, 1, MPI_C_COMPLEX, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_c_complex, 1, MPI_C_COMPLEX, 0, 0, MPI_COMM_WORLD);
  }
  free(buf);
