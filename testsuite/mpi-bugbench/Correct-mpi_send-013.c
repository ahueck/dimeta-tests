...
int main(int argc, char **argv) {
  ...
  unsigned long long int *buf_mpi_unsigned_long_long =
      (unsigned long long int *)calloc(1, sizeof(unsigned long long int));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_unsigned_long_long, 1, MPI_UNSIGNED_LONG_LONG, 1, 0,
             MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_unsigned_long_long, 1, MPI_UNSIGNED_LONG_LONG, 0, 0,
             MPI_COMM_WORLD);
  }
  free(buf);
