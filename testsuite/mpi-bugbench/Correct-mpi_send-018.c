...
int main(int argc, char **argv) {
  ...
  int8_t *buf_mpi_int8_t = (int8_t *)calloc(1, sizeof(int8_t));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_int8_t, 1, MPI_INT8_T, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_int8_t, 1, MPI_INT8_T, 0, 0, MPI_COMM_WORLD);
  }
  free(buf);
