...
/* ///////////////////////// The MPI Bug Bench ////////////////////////
  ...
  Description: datatype missmatch: Buffer: MPI_INT8_T MPI_Call: MPI_INT
...
int main(int argc, char **argv) {
  ...
  int8_t *buf_mpi_int8_t = (int8_t *)calloc(10, sizeof(int8_t));

  signed int *buf_mpi_int = (signed int *)calloc(10, sizeof(signed int));

  if (rank == 0) {
    /*MBBERROR_BEGIN*/ MPI_Recv(buf_mpi_int8_t, 10, MPI_INT, 1, 0,
                                MPI_COMM_WORLD,
                                MPI_STATUS_IGNORE); /*MBBERROR_END*/
  }
  if (rank == 1) {
    /*MBBERROR_BEGIN*/ MPI_Send(buf_mpi_int8_t, 10, MPI_INT, 0, 0,
