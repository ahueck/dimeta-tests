...
int main(int argc, char **argv) {
  ...
  signed short int *buf_mpi_short =
      (signed short int *)calloc(1, sizeof(signed short int));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_short, 1, MPI_SHORT, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_short, 1, MPI_SHORT, 0, 0, MPI_COMM_WORLD);
  }
  free(buf);
