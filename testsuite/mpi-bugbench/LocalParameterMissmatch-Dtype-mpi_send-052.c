...
/* ///////////////////////// The MPI Bug Bench ////////////////////////
  ...
  Description: datatype missmatch: Buffer: MPI_INT MPI_Call: MPI_UINT8_T
...
int main(int argc, char **argv) {
  ...
  uint8_t *buf_mpi_uint8_t = (uint8_t *)calloc(10, sizeof(uint8_t));

  if (rank == 0) {
    /*MBBERROR_BEGIN*/ MPI_Recv(buf_mpi_int, 10, MPI_UINT8_T, 1, 0,
                                MPI_COMM_WORLD,
                                MPI_STATUS_IGNORE); /*MBBERROR_END*/
  }
  if (rank == 1) {
    /*MBBERROR_BEGIN*/ MPI_Send(buf_mpi_int, 10, MPI_UINT8_T, 0, 0,
                                MPI_COMM_WORLD); /*MBBERROR_END*/
  }
  free(buf);
  free(buf_mpi_int);
  free(buf_mpi_uint8_t);
