...
/* ///////////////////////// The MPI Bug Bench ////////////////////////
  ...
  Description: datatype missmatch: Buffer: MPI_UNSIGNED_SHORT MPI_Call: MPI_INT
...
int main(int argc, char **argv) {
  ...
  if (rank == 0) {
    /*MBBERROR_BEGIN*/ MPI_Recv(buf_mpi_unsigned_short, 10, MPI_INT, 1, 0,
                                MPI_COMM_WORLD,
                                MPI_STATUS_IGNORE); /*MBBERROR_END*/
  }
  if (rank == 1) {
    /*MBBERROR_BEGIN*/ MPI_Send(buf_mpi_unsigned_short, 10, MPI_INT, 0, 0,
