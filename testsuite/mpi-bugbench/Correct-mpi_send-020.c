...
int main(int argc, char **argv) {
  ...
  int32_t *buf_mpi_int32_t = (int32_t *)calloc(1, sizeof(int32_t));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_int32_t, 1, MPI_INT32_T, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_int32_t, 1, MPI_INT32_T, 0, 0, MPI_COMM_WORLD);
  }
  free(buf);
