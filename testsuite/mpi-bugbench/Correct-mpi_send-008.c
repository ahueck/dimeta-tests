...
int main(int argc, char **argv) {
  ...
  signed char *buf_mpi_signed_char =
      (signed char *)calloc(1, sizeof(signed char));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_signed_char, 1, MPI_SIGNED_CHAR, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_signed_char, 1, MPI_SIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
  }
  free(buf);
