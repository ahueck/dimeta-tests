...
int main(int argc, char **argv) {
  ...
  uint16_t *buf_mpi_uint16_t = (uint16_t *)calloc(1, sizeof(uint16_t));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_uint16_t, 1, MPI_UINT16_T, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_uint16_t, 1, MPI_UINT16_T, 0, 0, MPI_COMM_WORLD);
  }
  free(buf);
