...
int main(int argc, char **argv) {
  ...
  wchar_t *buf_mpi_wchar = (wchar_t *)calloc(1, sizeof(wchar_t));

  int *buf = (int *)calloc(10, sizeof(int));

  if (rank == 0) {
    MPI_Recv(buf_mpi_wchar, 1, MPI_WCHAR, 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  }
  if (rank == 1) {
    MPI_Send(buf_mpi_wchar, 1, MPI_WCHAR, 0, 0, MPI_COMM_WORLD);
  }
  free(buf);
