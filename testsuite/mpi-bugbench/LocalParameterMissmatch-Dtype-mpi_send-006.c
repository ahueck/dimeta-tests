...
/* ///////////////////////// The MPI Bug Bench ////////////////////////
  ...
  Description: datatype missmatch: Buffer: MPI_LONG_LONG_INT MPI_Call: MPI_INT
...
int main(int argc, char **argv) {
  ...
  signed long long int *buf_mpi_long_long_int =
      (signed long long int *)calloc(10, sizeof(signed long long int));

  signed int *buf_mpi_int = (signed int *)calloc(10, sizeof(signed int));

  if (rank == 0) {
    /*MBBERROR_BEGIN*/ MPI_Recv(buf_mpi_long_long_int, 10, MPI_INT, 1, 0,
                                MPI_COMM_WORLD,
                                MPI_STATUS_IGNORE); /*MBBERROR_END*/
  }
  if (rank == 1) {
    /*MBBERROR_BEGIN*/ MPI_Send(buf_mpi_long_long_int, 10, MPI_INT, 0, 0,
                                MPI_COMM_WORLD); /*MBBERROR_END*/
  }
  free(buf);
  free(buf_mpi_long_long_int);
  free(buf_mpi_int);
